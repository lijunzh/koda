...
// ── String-returning variants for TUI renderer ────────────────
//
// These return formatted strings instead of printing directly,
// enabling output routing through `insert_before()` in the TUI.

/// Format a tool call banner as a string.
pub fn format_tool_call_banner(tc: &ToolCall, is_sub_agent: bool) -> String {
    let indent = if is_sub_agent { "  " } else { "" };
    let (color, label, detail) = tool_info(&tc.function_name, &tc.arguments);

    if tc.function_name == "ShareReasoning" {
        return format!("  \x1b[90m\u{1f4ad} Thinking...\x1b[0m");
    }

    let mut result = if tc.function_name == "Bash" {
        format!("\n{indent}{color}\u{25cf}{RESET} {BOLD}{label}{RESET} {detail}")
    } else {
        format!("\n{indent}{color}\u{25cf}{RESET} {BOLD}{label}{RESET} {DIM}{detail}{RESET}")
    };
    ...
    if tc.function_name == "Bash" {
        ...
        let timeout = args.get("timeout").and_then(|v| v.as_u64()).unwrap_or(60);
        let cwd = args.get("cwd").and_then(|v| v.as_str());
        result.push_str(&format!("\n{indent}  {DIM}\u{23f1} Timeout: {timeout}s"));
        if let Some(dir) = cwd {
            result.push_str(&format!("  \u{1f4c2} {dir}"));
        }
...
/// Format tool output as a string (collapsed view).
pub fn format_tool_output_str(tool_name: &str, output: &str) -> String {
    // Simplified: just return the output with basic formatting.
    // The full rendering logic in print_tool_output is complex —
    // this gives a reasonable summary until we migrate fully.
    if output.is_empty() || tool_name == "ShareReasoning" {
        return String::new();
    }
    let lines: Vec<&str> = output.lines().collect();
    ...
    if total <= 4 {
        lines
            .iter()
            .map(|l| format!("{CONTENT_INDENT}{DIM}\u{2502}{RESET} {l}"))
            .collect::<Vec<_>>()
            .join("\n")
    } else {
        let mut result = Vec::new();
        for line in &lines[..2] {
            result.push(format!("{CONTENT_INDENT}{DIM}\u{2502}{RESET} {line}"));
        }
        result.push(format!(
            "{CONTENT_INDENT}{DIM}\u{2502} ... ({} more lines){RESET}",
            total - 2
        ));
        result.join("\n")
    }
...
/// Format full tool output as a string.
pub fn format_tool_output_full_str(record: &ToolOutputRecord) -> String {
    ...
    for line in record.output.lines() {
        result.push_str(&format!("\n{CONTENT_INDENT}{DIM}\u{2502}{RESET} {line}"));
    }
    result
}

/// Format compact tool output as a string.
pub fn format_tool_output_compact_str(tool_name: &str, output: &str) -> String {
    if output.is_empty() {
        return String::new();
    }
    let lines = output.lines().count();
    format!("{CONTENT_INDENT}{DIM}\u{2514} {tool_name}: {lines} line(s){RESET}")
}
