#!/usr/bin/env bash
# Bootstrap (and sync) peer reference repos for koda development.
#
...
# Works as both bootstrap (first run on a new machine) and sync (daily
# refresh). One script, one mental model.
#
...
# ── Worker: bootstrap or update one peer ─────────────────────────────
process_peer() {
  local name="$1" url="$2"
