...
### Added (#1366 epic phase 1 — sub-agent rewrite scaffold)

- **`execute_sub_agent_sync` + `KODA_SUBAGENT_SYNC` env-var kill switch** (#1366 phase 1). Adds the public sync-dispatch entry point that the rest of the #1366 epic builds on, plus the migration-window opt-in flag. With `KODA_SUBAGENT_SYNC=1` set, `InvokeAgent` and `SpawnAgent` route through the existing FG inference path (`inline_only=true` recursion-guard branch promoted to a public symbol) instead of the legacy bg-spawn path from #1163 — the sub-agent runs synchronously and returns its final output as the tool result, no `task_id` round-trip via `WaitForMail`. Default is unset — the legacy bg-spawn path is unchanged for normal users; the flag exists for end-to-end testing of the new path before phase 2 flips the default. Phase 3 deletes the legacy branch + `WaitForMail` + mailbox + child-activity pill + overlay (~5,000 LOC), phase 6–7 add the inline `SubAgentBlock` / `SubAgentGroup` widgets that codex's ExecCell + gemini-CLI's `SubagentGroupDisplay` patterns inspire. The wrapper is intentionally a thin call to `execute_sub_agent(..., inline_only=true)` so phase 2 is a one-line callsite swap and the diff is trivial to review/revert. Tests: new `sync_dispatch_tests::subagent_sync_env_var_parsing` pins the truthy/falsy parsing (single `#[test]` because `cargo test` parallelizes by default and the flag is process-global); existing 1397 `koda-core` lib tests + 12 `e2e_agent_test` cases pass with the flag unset (default behavior unchanged). With the flag SET, six `e2e_agent_test` cases fail because they assert on the legacy bg-spawn registry's terminal-status `ChildTaskUpdate` events that the FG path deliberately doesn't emit (`sub_agent_dispatch.rs:1075` documents this); migrating those assertions is phase 2 work, not phase 1. Wire format unchanged — ACP / headless clients are unaffected.
