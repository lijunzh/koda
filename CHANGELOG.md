...
- **Slash commands typed during inference no longer silently steer the model** (#1211). The inference Enter (`QueueNext`) and Ctrl+J ("later") handlers in `tui_handlers_inference.rs` sent textarea contents straight to the engine without checking for a leading `/`. So `/cancel agent:1` typed during a `WaitTask` would arrive at the model as a raw user message — the bg agent kept running, the user got no feedback, and `📥 Next: /cancel agent:1` would render in scrollback as if it were a real steer. Both paths now consult a small classifier (`is_slash_command_attempt`) that trims leading whitespace and matches `/`, then push a visible warning (`Slash commands are disabled during inference. Press Esc to interrupt, then run the command at idle.`) instead of queueing. Behavior matches the design settled in #1210/#1213: the always-on bg-activity overlay covers status, `/cancel` covers selective cancel at idle, Esc covers cancel-all during inference.

- **Composer now shows a visible cursor at the current input position** (#1220). The textarea exposed a `cursor_pos(area)` API matching codex's `Renderable::cursor_pos`, but `tui_viewport::draw_viewport` never called `Frame::set_cursor_position` to consume it — so ratatui's default cursor-hidden state stuck and users had no visual indicator of where the next keystroke would land. Worse, **CJK/IME preedit was broken** (composition rendered at the screen origin instead of the caret) and **screen readers / magnifiers couldn't track the input position** (WCAG 2.1.1 / 2.4.7). Cross-referenced codex (`Frame::set_cursor_position`), claude-code (`useDeclaredCursor`) and gemini-cli (`chalk.inverse(' ')`); 2 of 3 use the hardware cursor and the third paints a software fallback — koda did neither. Fix mirrors codex: after rendering the textarea, call `frame.set_cursor_position((x, y))` with the screen coords from `cursor_pos_with_state`. Cursor *style* (vim block vs. bar via crossterm `SetCursorStyle`) is intentionally a follow-up.
